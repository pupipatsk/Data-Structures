...
class list
{
  ...
  public:
    ...
    void splitList(list<T>& list1, list<T>& list2) {
      ...
      size_t first_HalfSize = mSize / 2;
      if (mSize % 2 != 0) first_HalfSize++;

      node *f1 = mHeader->next;
      node *l1 = f1;
      for (int i = 0; i < first_HalfSize-1; i++) l1 = l1->next;
      node *f2 = l1->next;
      node *l2 = mHeader->prev;

      node *list1_last = list1.mHeader->prev;
      list1_last->next = f1;
      f1->prev = list1_last;
      l1->next = list1.mHeader;
      list1.mHeader->prev = l1;

      if (f2 != mHeader) {
        node *list2_last = list2.mHeader->prev;
        list2_last->next = f2;
        f2->prev = list2_last;
        l2->next = list2.mHeader;
        list2.mHeader->prev = l2;
      }
