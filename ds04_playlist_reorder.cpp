...
class list
{
  ...
  public:
    ...
    void reorder(int pos,std::vector<int> selected) {
      ...
      iterator it = begin();
      size_t i = 0, idx = 0;
      std::vector<iterator> selectedDataIt;
      while (i <= selected[selected.size()-1])
      {
        if (i == selected[idx]) {
          selectedDataIt.push_back(it);
          idx++;
        }
        it++;
        i++;
      ...
      if (pos == 0) { // push_front
        for (int i = selectedDataIt.size()-1; i >= 0; i--) {
          push_front(*selectedDataIt[i]);
        }
      }
      else if (pos == mSize) { // push_back
        for (auto& eIt : selectedDataIt) {
          push_back(*eIt);
        }
      }
      else {
